...
@dataclass
class DataCollatorForMaskRouge:
    """
    Data collator that will dynamically pad the inputs received, as well as the labels.
    ...
    label_pad_token_id: int = -100
    return_tensors: str = "pt"
    mask_labels_ids_name = None

    def __call__(self, features, return_tensors=None):
        if return_tensors is None:
            return_tensors = self.return_tensors
        # label_names = ["labels", "mask_labels"]
        label_cache = {}
        label_names = [key for key in features[0].keys() if 'label' in key]
        for label_name in label_names:
        # label_name = self.mask_labels_ids_name
            labels = [feature[label_name] for feature in features] if label_name in features[0].keys() else None
            label_cache[label_name] = labels
            # We have to pad the labels before calling `tokenizer.pad` as this method won't pad them and needs them of the
            # same length to return tensors.
            if labels is not None:
                max_label_length = max(len(l) for l in labels)
                ...
                padding_side = self.tokenizer.padding_side
                for feature in features:
                    remainder = [self.label_pad_token_id] * (max_label_length - len(feature[label_name]))
                    if isinstance(feature[label_name], list):
                        feature[label_name] = (
                            feature[label_name] + remainder if padding_side == "right" else remainder + feature[label_name]
                        )
                    elif padding_side == "right":
                        feature[label_name] = np.concatenate([feature[label_name], remainder]).astype(np.int64)
                    else:
                        feature[label_name] = np.concatenate([remainder, feature[label_name]]).astype(np.int64)
        ...
        for label_name in label_names:
            if (
                label_cache[label_name] is not None
                and self.model is not None
                and hasattr(self.model, "prepare_decoder_input_ids_from_labels")
            ):
                decoder_input_ids = self.model.prepare_decoder_input_ids_from_labels(labels=features[label_name])
                features[f"{label_name}_decoder_input_ids"] = decoder_input_ids

        return features


@dataclass
class DataCollatorForBartForConditionalGenerationWithRouge:
    """
    Data collator that will dynamically pad the inputs received, as well as the labels.
    ...
    def __call__(self, features, return_tensors=None):
        if return_tensors is None:
            return_tensors = self.return_tensors
        multi_label_names = [key for key in features[0].keys() if "label" in key]
        multi_label_names = ["labels"]
        for label_name in multi_label_names:
            labels = [feature[label_name] for feature in features] if label_name in features[0].keys() else None
            # We have to pad the labels before calling `tokenizer.pad` as this method won't pad them and needs them of the
            # same length to return tensors.
            if labels is not None:
...
@dataclass
class DataCollatorForMaskPromptLanguageModeling(DataCollatorMixin):
    """
    Data collator used for language modeling. Inputs are dynamically padded to the maximum length of a batch if they
    are not all of the same length.
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if isinstance(examples[0], Mapping):
            ...
            for example in examples:
                del example['labels']
                del example['special_tokens_mask']
            batch = self.tokenizer.pad(examples, return_tensors="pt", pad_to_multiple_of=self.pad_to_multiple_of)
            labels = [label + [-100 for _ in range(batch["input_ids"].shape[1] - len(label))] for label in labels]
